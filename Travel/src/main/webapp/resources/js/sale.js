$(function() {
	...
	$('#sale_btn').click(function() {
		...
		var dt_price = [];
		var dt_name = [];
		$("input[name='dt_pdt_id']").each(function(i){ // 상품의 추가 요청 배열에 값 추가
			dt_pdt_id.push($("input[name='dt_pdt_id']").eq(i).val());
			dt_count.push($("input[name='dt_count']").eq(i).val());
			dt_price.push(deleteComma($("input[name='dt_price']").eq(i).val()));
			dt_name.push($("input[name='dt_pdt_id']").eq(i).closest('tr').text());
...
//		
	...
	$('ul.manu_sel li').click(function() {
		var pdt_id = $(this).data("hidden");var pdt_name = $(this).data("options").name;var pdt_cost = $(this).data("options").cost;var order_price = pdt_cost/1;
		var changeId = 0;var totalCount = 1;var check = false;
		// 주문리스트에 해당 메뉴 있는가? 판별
		$("input[name='pdt_id']").each(function(i) {
			if(pdt_id==$("input[name='pdt_id']").eq(i).val()){
				check = true;	changeId = i;
			};	// 주문 총액 계산기
		...
		$("input[name='dt_price']").each(function(i) {
			var plus_dt_price = parseInt(deleteComma($(this).val()));		dt_price += plus_dt_price 
		})
		var totalItems = $("input[name='pdt_price']").length;	order_price += dt_price;	var point = parseInt($("#tlt_dist").val());
			$('#total_items').val(totalItems);
			$('#total_qty').val(totalCount);
