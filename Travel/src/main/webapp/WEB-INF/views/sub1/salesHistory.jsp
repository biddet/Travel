			...
			<div class="form-group">
			  ...
			  <div class="col-md-2">
				<select name="cancel_search" id="cancel_search" class="form-control">
					...
					<option value="1" <c:if test="${param.cancel_search == 1 }">selected</c:if>>판매저장</option>
					<option value="2" <c:if test="${param.cancel_search == 2 }">selected</c:if>>판매취소</option>
				</select>
				</select>
			  </div>
			  <div class="col-md-1">
                ...
                <tr id="tr_top">
  				...
  				<c:choose>
	  				<c:when test="${list != null}">
						...
						<td>${list.pot_id }</td>
						<td><c:choose><c:when test="${list.ord_cancel == 0}">판매완료</c:when><c:when test="${list.ord_cancel == 1}">판매저장</c:when><c:otherwise>판매취소</c:otherwise></c:choose></td>
